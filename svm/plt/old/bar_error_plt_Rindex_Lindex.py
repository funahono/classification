...
for subject in subject_list:
    ...
    #current data
    cur_read_file = f'{read_dir}/{subject}/svm/current/current_{subject}_Rindex_vs_Lindex_classification_report.csv'
    ...
    #eeg data
    eeg_read_file = f'{read_dir}/{subject}/svm/EEG_{comment}/EEG_{subject}_Rindex_vs_Lindex_classification_report.csv'
    eeg_df = pd.read_csv(eeg_read_file, index_col=0, encoding='utf-8-sig') * 100

    eeg_acc = eeg_df.loc['average_result', key]
    # eeg_acc = eeg_df.loc['average_result', 'precision']
...
cur_std = np.std(cur_acc_list)
eeg_std = np.std(eeg_acc_list)
...
plt.errorbar(len(subject_list) + 1 - width/2, eeg_avg_acc, yerr = eeg_std, fmt = 'o' , makersize = color = 'red' , capsize=5, capthick=2)

plt.xticks(x, label_list)

if key == 'accuracy':
    plt.ylabel('Accuracy (%)')
    save_file_name = 'Rindex_vs_Lindex_all_sub_acc_error.png'

elif key == 'f1-score':
    plt.ylabel('F1-Score (%)')
    save_file_name = 'Rindex_vs_Lindex_all_sub_fscore_error.png'
