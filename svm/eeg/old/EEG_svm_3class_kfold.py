...
save_dir = f'/media/honoka/HDD1/Funatsuki/Experiment/{day_sub}/svm_3class/EEG_{comment}_k={k_num}_{kernel_type}'
...
data = np.zeros((20*3*class_num, 20)) #(60 × 3 class, 20 channnels)
...
for k in range(k_num):
    ...
    # set modelkun
    modelkun = SVC(kernel=kernel_type,decision_function_shape=multi_classfication_type,break_ties=True)
    ...
    ########################### save png ##########################################
    #save paht
    save_confusion_png_path = os.path.join(save_dir, f'EEG_{day_sub}_3class_{f_range[0]}_{f_range[1]}Hz_{t_range[0]}_{t_range[1]}s_{k+1}_in_{k_num}.png')
